...
def load_known_faces_from_db():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT display_name, age, function_text, person_id_code, image_filename, face_encoding 
        FROM persons
    ''')
...
def initialize_ppe_model():
    ...
    try:
        ...
        print(f"Error initializing PPE model: {e}")
        return False

def detect_faces_and_ppe(frame):
    """Detect faces and PPE in frame"""
    if frame is None:
        return frame
